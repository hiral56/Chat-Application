    ...
    io.on('connection', socket => {
        ...
        socket.on('join-user', userName => {
            ...
            const user = {
                id: socket.id,
                name: userName,
                imageSrc: "https://robohash.org/"+userName+"?set=set4",
                isLoggedIn: false
            }

            for(var i in users)
            {
                  var user_to_lower = users[i].name.toLowerCase();
                  var userName_to_lower = userName.toLowerCase();
                  console.log();
                  if(user_to_lower === userName_to_lower){
